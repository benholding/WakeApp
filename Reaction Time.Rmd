...
packages <- c("data.table", "lubridate", "anytime", "dplyr", "ggplot2", "nlme", "tidyr", "nlme", "sjPlot","jtools","ggthemes", "xts") #These are the extra packages needed to merge the CSV files
...
#working out actual timings
time_stamps <- as.POSIXct(KSS_Complete_TIDY$t0/1000, origin = "1970-01-01")
t <- strftime(time_stamps, format="%H:%M")
KSS_Complete_TIDY$t0 <- hm(t)
KSS_Complete_TIDY$t0 <- hour(KSS_Complete_TIDY$t0 ) + minute(KSS_Complete_TIDY$t0 )/60 + second(KSS_Complete_TIDY$t0 )/3600
RTactualsleeptimes <- KSS_Complete_TIDY %>% group_by(time,SD) %>% summarise(Circadian_mean = circadian.mean(t0,na.rm=T),Circadian_sd=circadian.sd(t0,na.rm=T)[[2]])
...
```{r}
KSS_Complete_TIDY <- KSS_Complete_TIDY[which(KSS_Complete_TIDY$reaction_time > 100 & KSS_Complete_TIDY$reaction_time < 1000),]
```
...
```{r}
KSS_Complete_TIDY$SD <- factor(KSS_Complete_TIDY$SD, labels = c("Control", "Sleep Deprivation"))
...
ggplot(data = KSS_Complete_TIDY,  aes(time, reaction_time, colour=SD)) + 
...
ggplot(KSS_Complete_TIDY, aes(time, reaction_time, colour=SD)) + 
  ...
  theme_bw() + stat_summary(aes(y=fitted(RT.final.model)), fun.data=mean_cl_normal, fun.args=list(conf.int=0.95), geom="pointrange") + stat_smooth(method = "lm", formula  =y ~ poly(x,3)) + geom_point(data=KSS_Complete_TIDY,aes(time, reaction_time, colour=SD), position = position_jitterdodge(jitter.width = 0.5, jitter.height=0.4, 
                                dodge.width=0.9))
theme_set(theme_tufte())
plot_model(RT.final.model, type = "pred", terms = c("time", "SD"),
           title = "",
           axis.title = c("Time Point","Reaction Time (ms)"))
```
